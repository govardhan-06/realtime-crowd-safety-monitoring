...
export default async function IncidentsPage() {
  ...
  return <main className="shell">
    <header className="masthead"><div><p className="eyebrow">Human review console / M5</p><h1>Detected indicators</h1><p className="lede">One evolving incident per source and zone. Review the evidence before choosing an action.</p><Link className="button primary" href="/process">Process a video</Link></div><div className="live-mark"><span aria-hidden="true" /> Review API {health.status === 'ok' ? 'online' : 'unavailable'}</div></header>
