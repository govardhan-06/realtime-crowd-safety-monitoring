...
- `flows.jsonl` — one record per configured ROI/LOI with interval inflow, outflow, and net-flow rates when LOIs are configured.
...
`colab-notebooks/m6_evaluation_demo.ipynb` reports X3D component metrics per dataset for held-out `test` and external `external_test` rows. The standalone X3D notebook records decode/buffer diagnostics and coverage, selects one threshold plus video aggregation rule from `validation`, saves that selection/provenance, and applies it unchanged to test/external rows. Its full incident replay uses only reviewed long-form UBI-Fights `test` entries from the separate incident manifest and emits A/B/C/D crowd-signal ablation rows traced to run/config/feature versions. Local notebook checks validate contracts only; Colab media/model results remain pending until manually run.
